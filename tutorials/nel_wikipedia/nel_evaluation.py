...
def measure_performance(dev_data, kb, el_pipe, baseline=True, context=True, dev_limit=None):
    ...
    for doc, gold in tqdm(dev_data, total=dev_limit, leave=False, desc='Processing dev data'):
        if len(doc) > 0:
            ...
            for entity in gold["entities"]:
                ...
                offset = (start, end)
                for gold_kb, value in gold["links"][offset].items():
                    if value:
                        # only evaluating on positive examples
                        correct_ents[offset] = gold_kb
            nlp_doc = el_pipe(doc)
            if baseline:
                _add_baseline(baseline_results, counts, nlp_doc, correct_ents, kb)
            if context:
                # using only context
                el_pipe.config["incl_context"] = True
                el_pipe.config["incl_prior"] = False
                _add_eval_result(context_results, doc, correct_ents, el_pipe)

                # measuring combined accuracy (prior + context)
                el_pipe.config["incl_context"] = True
                el_pipe.config["incl_prior"] = True
    ...
    if baseline:
        ...
        print(baseline_results.report_performance("random"))
        print(baseline_results.report_performance("prior"))
        print(baseline_results.report_performance("oracle"))

    if context:
        logger.info(context_results.report_metrics("context only"))
        logger.info(combo_results.report_metrics("context and prior"))
        print(context_results.report_metrics("context only"))
        print(combo_results.report_metrics("context and prior"))


def _add_eval_result(results, doc, correct_ents, nlp):
    """
